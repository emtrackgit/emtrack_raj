...
class EditTyreService {
  ...
  static Future<void> updateTyre(EditTyreModel model) async {
    try {
      ...
      if (isOffline) {
        final tireId = payload['tireId'] as int?;
        ...
        if (idx >= 0) {
          tyres[idx] = {...tyres[idx], ...payload, 'syncStatus': 'pending', '_localOnly': true};
          await LocalStorageService.saveTyres(tyres);
        }

        print("✅ Offline tyre update queued tireId=$tireId");
        return;
      }
