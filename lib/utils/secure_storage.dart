...
class SecureStorage {
  ...
  static const _pendingCreateTyreQueueKey = 'pending_create_tyre_queue';
  static const _pendingEditTyreQueueKey = 'pending_edit_tyre_queue';

  static Future<List<String>> getPendingCreateTyreQueue() async {
    final raw = await _storage.read(key: _pendingCreateTyreQueueKey);
  ...
  static Future<void> enqueuePendingCreateTyre(String requestBodyJson) async {
    final list = await getPendingCreateTyreQueue();
    list.add(requestBodyJson);
    await _storage.write(
      key: _pendingCreateTyreQueueKey,
      value: jsonEncode(list),
    );
  }

  static Future<void> removePendingCreateTyreAt(int index) async {
    final list = await getPendingCreateTyreQueue();
  ...
  static Future<List<String>> getPendingEditTyreQueue() async {
    final raw = await _storage.read(key: _pendingEditTyreQueueKey);
    if (raw == null || raw.trim().isEmpty) return <String>[];
    try {
      final decoded = jsonDecode(raw);
      if (decoded is List) {
  ...
  static Future<void> enqueuePendingEditTyre(String requestBodyJson) async {
    final list = await getPendingEditTyreQueue();
    list.add(requestBodyJson);
    await _storage.write(
      key: _pendingEditTyreQueueKey,
      value: jsonEncode(list),
    );
  }

  static Future<void> removePendingEditTyreAt(int index) async {
    final list = await getPendingEditTyreQueue();
    if (index < 0 || index >= list.length) return;
    list.removeAt(index);
    await _storage.write(
