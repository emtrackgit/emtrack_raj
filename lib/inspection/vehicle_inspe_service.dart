...
class VehicleInspeService {
  ...
  Future<bool> saveVehicleFootPrintDetails({
    required List<Map<String, dynamic>> tireList,
  }) async {
    try {
      final headers = await SecureStorage.authHeaders();

      final url = Uri.parse(
        "${ApiConstants.baseUrl}/api/Inspection/SaveVehicleFootPrintDetails",
      );

      final body = jsonEncode({"tireList": tireList});
      print("🟢 ROTATE PUT URL => $url");
      print("🟢 ROTATE HEADERS => $headers");
      print("📤 ROTATE BODY => $body");
      ...
      print("🟡 ROTATE STATUS => ${response.statusCode}");
      print("🟡 ROTATE RESP => ${response.body}");

      if (response.statusCode == 401 || response.statusCode == 403) {
        print("❌ ROTATE Unauthorized");
        return false;
      }
      ...
      if (decoded is Map<String, dynamic>) {
        if (decoded['didError'] == true) {
          print("❌ ROTATE didError => ${decoded['errorMessage']}");
          return false;
        }
        return true;
      }
